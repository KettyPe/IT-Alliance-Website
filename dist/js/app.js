(() => {
    "use strict";
    const desctop = window.innerWidth >= 1280;
    const tablet = window.innerWidth < 1280;
    const smallTablet = window.innerWidth < 991.98;
    ...
    slides.forEach((slide => {
        ...
        buttons.forEach((btn => {
            if (listItems.length <= 3) btn.style.display = "none";
        }));
    }));
    if (dropdowns) dropdowns.forEach((dropdown => {
        dropdown.addEventListener("click", (event => {
            const openDropdown = document.querySelector(".open-dropdown");
            if (openDropdown && openDropdown !== dropdown) openDropdown.classList.remove("open-dropdown");
            dropdown.classList.toggle("open-dropdown");
        }));
        document.addEventListener("click", (event => {
            if (!dropdown.contains(event.target)) dropdown.classList.remove("open-dropdown");
        }));
    }));
    ...
    if (filterCatalogItem) filterCatalogItem.forEach((catalogItem => {
        ...
        if (items.length > 7) {
            ...
            for (let i = 7; i < items.length; i++) items[i].style.display = "none";
            listsBtn.addEventListener("click", (() => {
                ...
                if (isExpanded) {
                    ...
                    for (let i = 7; i < items.length; i++) items[i].style.display = "block";
    ...
    promoSpoller?.addEventListener("click", (() => {
        ...
        if (promoSpoller.classList.contains("open-spoller")) promoInput.style.maxHeight = promoInput.scrollHeight + 20 + "px"; else promoInput.style.maxHeight = null;
    }));
    ...
    steppers.forEach((stepper => {
        ...
        const changesClassDisabled = () => {
            if (count == 1) stepperBtnDown.classList.add("stepper__btn--disable"); else stepperBtnDown.classList.remove("stepper__btn--disable");
        };
    ...
    btnProperty?.addEventListener("click", (e => {
        ...
        if (btnProperty.classList.contains("open-property")) {
            for (const element of property) if (element.classList.contains("hidden")) {
    ...
    inputs.forEach((input => {
        ...
        function handleInputChange() {
            if (input.value) label.classList.add("top-title"); else label.classList.remove("top-title");
        }
    }));
    ...
    toggleBtn?.addEventListener("click", (() => {
        ...
        if (toggleBtn.classList.contains("active")) configuratorWrapper.classList.add("close"); else configuratorWrapper.classList.remove("close");
    }));
    document.querySelectorAll(".slide-configurator__screen-wrapper").forEach((image => {
        image.addEventListener("click", (function(event) {
            const clickedImage = event.target;
            if (clickedImage) console.log(`Вы нажали на: ${clickedImage.alt}`);
        }));
    }));
    document.querySelectorAll(".config-sreen-info-wrapper");
    ...
    menuIcon.addEventListener("click", (() => {
        ...
        if (menuBody.classList.contains("menu-open")) disableScroll(); else {
    ...
    function subMenuActions(submenuWrapper, link) {
        submenuWrapper.forEach((wrapper => {
            ...
            wrapper.querySelector(".submenu__sublist");
            let currentBtn = null;
            wrapper.addEventListener("mousemove", (e => {
                ...
                setTimeout((() => {
                    submenuBtns.forEach((btn => {
                        ...
                        if (e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom) currentBtn = btn;
                        const activeSubBtn = link.querySelector(".submenu__item.submenu__item--active");
                        if (currentBtn) {
    ...
    subMenuBtns.forEach((subBtn => {
        if (tablet) {
            ...
            subBtn.addEventListener("click", (() => {
                ...
                if (subBtn.classList.contains("active")) children.classList.add("open"); else children.classList.remove("open");
            }));
        }
    }));
    function moveMenu() {
        menuLinks.forEach((link => {
            link.addEventListener("click", (event => {
                ...
                if (window.innerWidth < 1280) link.querySelector(".submenu__body")?.classList.add("open");
            }));
        }));
        if (tablet) subMenuCloseBtn.forEach((closeBtn => {
    ...
    document.querySelectorAll(".dropdown").forEach((function(dropDownWrapper) {
        ...
        dropDownListItems.forEach((function(listItem) {
            listItem.addEventListener("click", (function(e) {
                ...
                } else dropDownTitle.classList.remove("top-title");
            }));
        }));
    ...
    if (tabsEquipments) tabsEquipments.forEach((tabsEquipment => {
        ...
        const tabsHandler = path => {
            tabsEquipmentContent.forEach((el => {
                el.classList.remove("tabs-active-content");
            }));
    ...
    function moveContent() {
        filtersSelected?.forEach((selected => {
            const selectedInner = selected.querySelector(".selected-filter-catalog__items");
            const arraySelectedWrapper = selected.querySelectorAll(".selected-filter-catalog__items");
            const showMoreSelected = selected.querySelector(".selected-filter-catalog__show-more");
            if (window.innerWidth >= 992) arraySelectedWrapper.forEach((array => {
                const arraySelected = array.querySelectorAll(".selected-filter-catalog__item");
                const resetSelected = array.querySelector(".selected-filter-catalog__reset");
                if (arraySelected.length <= 2) resetSelected.style.display = "none";
            }));
            if (window.innerWidth < 992) if (selectedInner.scrollHeight > 140) showMoreSelected?.addEventListener("click", (() => {
            ...
            })); else showMoreSelected.style.display = "none";
        }));
    }
    moveContent();
    ...
    search.querySelector(".search-header__button");
    ...
    formFileBody.forEach((form => {
        ...
        const filePreviewImg = form.querySelector(".file-actions-form__filePreviewImg");
        const filePreviewText = form.querySelector(".file-actions-form__filePreviewText");
        form.querySelector(".file-actions-form__perview");
        const formPerviewBtnClose = form.querySelector(".file-actions-form__perview-btn");
    ...
    ymaps.ready((function() {
        ...
        if (mapEkaterinburg) {
            const Ekaterinburg = new ymaps.Map("Ekaterinburg", {
            ...
            const myPlacemark = new ymaps.Placemark([ 56.84215706787041, 60.57993949999997 ], {}, {
        ...
        if (mapProjects) {
            const mapProjects = new ymaps.Map("mapProjects", {
                center: [ 56.84215706787041, 60.57993949999997 ],
                zoom: 16,
                controls: []
            }, {
                searchControlProvider: "yandex#search"
            }), MyIconContentLayout = ymaps.templateLayoutFactory.createClass('<div style="color: #101828; font-weight: bold; font-size: 16px;">$[properties.iconContent]</div>');
            ...
            const myPlacemark2 = new ymaps.Placemark([ 56.84176310981753, 60.58504642596431 ], {}, {
                iconLayout: "default#image",
                iconImageHref: "",
                iconImageSize: [ 60, 60 ],
                iconImageOffset: [ -50, -40 ],
    ...
    window.addEventListener("resize", (() => {
        if (window.innerWidth < 991.98) {
            new Swiper(".configurator__slider", {
                modules: [ Navigation ],
                slidesPerView: 1,
                spaceBetween: 15,
                speed: 800,
